...
Buildermark is an optional, disabled-by-default historical association helper. Agent Analytics does not embed its UI or runtime and never treats `linesFromAgent` as a quality, ownership, or score signal.

Run a gate explicitly with a local repository and sanitized evidence file:

```sh
agent-analytics buildermark-gate ./buildermark-evidence.json --repository /absolute/path/to/repository
```
