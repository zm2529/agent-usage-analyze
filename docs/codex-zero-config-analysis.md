...
Refresh only the Codex Hook:

```sh
agent-usage-analyze install-hook
```

The installer preserves unrelated Hook handlers, creates a private backup before changing an existing Codex Hook file, and is idempotent.
...
`awaiting-capability` is terminal until the capability changes or you explicitly run `agent-usage-analyze queue retry --all`; it is not a retry loop.
...
```sh
agent-usage-analyze config analysis --show
```

Select a mode explicitly:

```sh
agent-usage-analyze config analysis --mode auto
agent-usage-analyze config analysis --mode codex-native
agent-usage-analyze config analysis --mode provider
agent-usage-analyze config analysis --mode local-only
agent-usage-analyze config analysis --mode off
```
...
- `provider` and `codex-native` remain available as explicit choices. Explicit API-key Codex use is metered normally.

Manual workflows remain available:

```sh
agent-usage-analyze import-codex
agent-usage-analyze insights <session-id>
agent-usage-analyze queue process
agent-usage-analyze queue retry <session-id> --source codex-cli
```
...
A ChatGPT-authenticated Codex run uses the signed-in Codex subscription instead of a separately configured Provider API key. This does **not** mean unlimited or free usage: plan limits and Codex product terms still apply. Agent Usage Analyzer records input, cached input, output, reasoning tokens, and elapsed time as `codex-native` Observer Overhead. It does not invent a dollar cost. Provider/API-key modes retain their normal billing semantics.

All imported data, analysis results, queue state, and overhead records remain in the local Agent Usage Analyzer data directory. The dashboard listens on loopback only. Product telemetry stays disabled by default.

## Troubleshooting

- `codex-not-logged-in`: run `codex login`, verify `codex login status`, then `agent-usage-analyze queue retry --all`.
...
- `source-not-found`: run `agent-usage-analyze import-codex`, then retry with both session ID and `--source codex-cli`.
- `awaiting-capability`: read the downgrade reason in `queue status` or Dashboard; it will not retry until requested.
- `settled-analysis-failed`: inspect the private `settled-analysis.log` under the Agent Usage Analyzer config directory, fix the stated capability, then retry.
- Hook does not run: rerun `agent-usage-analyze doctor --verbose`, open Codex `/hooks`, and confirm the exact managed handler is trusted.
- Disable without uninstalling: `agent-usage-analyze config analysis --mode off`.
- Remove only Agent Usage Analyzer handlers: `agent-usage-analyze uninstall-hook`. Unrelated Hook handlers are preserved.
