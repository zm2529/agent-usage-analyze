...
function nextAction(reason: string | null, status: AutomaticAnalysisStatus['recentStatus']): string {
  if (reason === 'codex-not-logged-in') {
    return 'Run `codex login`, then `agent-analytics queue retry --all`.';
  }
  if (reason === 'codex-cli-missing') {
    return 'Install Codex CLI, sign in, then run `agent-analytics queue retry --all`.';
  }
  if (reason === 'codex-auth-unknown') {
    return 'Run `codex login status`; choose an explicit analysis mode if needed, then run `agent-analytics queue retry --all`.';
  }
  if (reason === 'provider-not-configured') {
    return 'Run `agent-analytics config llm`, then `agent-analytics queue retry --all`.';
  }
  if (reason === 'source-not-found' || reason === 'compatibility-projection-unavailable') {
    return 'Run `agent-analytics import-codex`, then retry this source and session from the queue.';
  }
  if (reason === 'settled-analysis-failed') {
    return 'Inspect `settled-analysis.log`, fix the reported capability, then run `agent-analytics queue retry --all`.';
  }
  if (reason === 'settled-import-failed') {
    return 'Run `agent-analytics import-codex`, inspect the import failure, then run `agent-analytics queue retry --all`.';
  }
  ...
  if (status === 'failed' || status === 'awaiting-capability') {
    return 'Resolve the reported downgrade, then run `agent-analytics queue retry --all`.';
  }
