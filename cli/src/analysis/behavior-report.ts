...
export const BEHAVIOR_REPORT_PROMPT_VERSION = 'behavior-report-v11';
...
export function buildBehaviorReportDataset(
  ...
  const observedInputTokenVolume = uncachedInputTokens + cacheCreationTokens + cacheReadTokens;
  const observedTokenVolume = observedInputTokenVolume + outputTokens;
  const tokenEfficiency: BehaviorReportDataset['tokenEfficiency'] = {
    inputTokens,
    outputTokens,
    cacheCreationTokens,
    cacheReadTokens,
    cacheReadShare: ratio(cacheReadTokens, observedInputTokenVolume),
...
const REPORT_SCHEMA = {
  ...
  properties: {
    ...
    tokenEfficiencyFindings: { type: 'array', items: { type: 'object', required: ['title', 'observation', 'savingMechanism', 'applicability', 'wasteType', 'signalLayer', 'attribution', 'confidence', 'evidenceRefs'], properties: {
      title: { type: 'string' }, observation: { type: 'string' }, savingMechanism: { type: 'string' },
      applicability: { type: 'string' },
      wasteType: { type: 'string', enum: ['cache-instability', 'redundant-context', 'retry-repair-loop', 'tool-result-bloat', 'model-mismatch', 'coordination-overhead', 'over-reasoning', 'repeated-task-reasoning', 'other'] },
      signalLayer: { type: 'string', enum: ['L0', 'L1', 'L2', 'L3'] },
      attribution: { type: 'string', enum: ['harness-waste', 'capability-limit', 'unknown'] },
