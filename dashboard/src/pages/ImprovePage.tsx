...
function EvidenceLinks({ refs }: { refs: string[] }) {
  ...
  const labels: Record<string, string> = {
    ...
    'tokenEfficiency.cacheReadShare': cn ? '缓存读取占比' : 'Cache-read share',
...
export default function ImprovePage() {
  ...
  return <div className="vibe-page">
    ...
    <section className="grid border-b md:grid-cols-5">
      ...
      <div className="py-5 md:pl-5"><p className="vibe-mono text-[10px] text-muted-foreground">{cn ? '代表性片段' : 'Representative episodes'}</p><p className="mt-2 text-sm font-semibold">{representativeSample?.count ?? dataset?.representativeEpisodes.length ?? 0}</p></div>
    </section>
    ...
    {!report && <section className="border-b py-12 text-center"><h2 className="vibe-serif text-2xl">{isGenerating ? t('analysis.generatingReport', 'Generating the cross-session LLM report…') : state.data?.needsRegeneration ? (cn ? '分析方法已更新' : 'Analysis method updated') : (cn ? '尚未生成使用分析' : 'Usage analysis has not been generated')}</h2><p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground">{isGenerating ? (cn ? '分析正在后台运行；离开本页不会中断。' : 'Analysis is running in the background; leaving this page will not interrupt it.') : state.data?.needsRegeneration ? (cn ? '可以现在重新分析，或等待下一次自动分析。' : 'Run analysis now or wait for the next automatic analysis.') : (cn && state.data?.eligibilityReason ? state.data.eligibilityReason : (cn ? '历史导入完成并具备足够会话后会自动开始首次分析。' : 'The first analysis starts automatically after history import has enough sessions.'))}</p></section>}
    ...
    {report && <section className="grid border-b lg:grid-cols-2">
      ...
      <div className="py-9 lg:pl-9"><h2 className="vibe-serif text-2xl">{cn ? 'Token 使用情况' : 'Token usage'}</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">{cn ? '在保留必要上下文、验证和安全信息的前提下，查找可以减少重复输入的地方。' : 'Find repeated input that can be reduced while preserving necessary context, validation, and safety information.'}</p><div className="mt-4 grid grid-cols-2 border-y text-xs"><div className="border-r p-3"><span className="text-muted-foreground">{cn ? '缓存读取占比' : 'Cache-read share'}</span><strong className="mt-1 block vibe-mono">{Math.round((dataset?.tokenEfficiency?.cacheReadShare ?? 0) * 100)}%</strong></div><div className="p-3"><span className="text-muted-foreground">{cn ? '发生压缩的会话' : 'Compacted sessions'}</span><strong className="mt-1 block vibe-mono">{dataset?.tokenEfficiency?.sessionsWithCompaction ?? 0}</strong></div></div><div className="border-t">{(report.tokenEfficiencyFindings ?? []).length ? (report.tokenEfficiencyFindings ?? []).map((item) => <AnalysisItem key={item.title} title={item.title}><p>{item.observation}</p><p className="mt-2 text-xs"><strong>{cn ? '如何减少重复：' : 'Reduce repetition: '}</strong>{item.savingMechanism}</p><p className="mt-2 text-xs"><strong>{cn ? '适用场景：' : 'Applies to: '}</strong>{item.applicability}</p><EvidenceLinks refs={item.evidenceRefs} /></AnalysisItem>) : <p className="border-b py-5 text-xs text-muted-foreground">{cn ? '当前没有可靠、且不损害质量的 Token 改进建议。' : 'No reliable token-saving recommendation is available without risking quality.'}</p>}</div></div>
    </section>}
